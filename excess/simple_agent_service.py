...
@app.post("/api/group/{group_id}/messages")
async def post_group_message(group_id: str, payload: dict):
    """POST a message into a group's message stream (used by frontend writers)

    payload: { sender_id: str, message: str, message_type: str }
    """
    if not supabase:
        raise HTTPException(status_code=500, detail="Supabase not configured")
    ...
    try:
        ...
        supabase.table('group_messages').insert({
            'group_id': group_id,
            'sender_id': sender_id,
            'sender_type': 'user' if sender_id != 'system' and sender_id != 'planner_agent' else 'system',
            'message': message,
        ...
        print(f"❌ Error posting message to group: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
Your custom itinerary has been generated. Check the messages below!"""
        ...
        # Add itinerary as separate message only if no itinerary exists yet (avoid duplicates)
        existing_itin = supabase.table('group_messages').select('*').eq('group_id', group_id).eq('message_type', 'itinerary').execute()
        if not existing_itin.data or len(existing_itin.data) == 0:
            supabase.table('group_messages').insert({
                'group_id': group_id,
            ...
            print(f"💬 Stored itinerary in Supabase")
        else:
            print(f"ℹ️ Itinerary message already exists for group {group_id}, skipping duplicate insertion")
