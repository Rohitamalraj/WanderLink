...
async function processIdentityDocument(formData, documentPath, walletAddress) {
  try {
    ...
    // Force OCR to always run and fail if not possible
    let parsedData = null;
    ...
    try {
      ...
      logger.info('✅ Document parsed successfully', { textLength: parsedData.text.length, confidence: parsedData.confidence });
      extractedData = extractIdentityData(parsedData);
      // Check for extracted document number and DOB
      const extractedPassport = (extractedData.passportNumbers && extractedData.passportNumbers[0]) || null;
      const extractedDOB = (extractedData.dates && extractedData.dates[0]) || null;
      if (!extractedPassport || !extractedDOB) {
        logger.error('❌ Could not extract document number or DOB from document');
        if (fs.existsSync(documentPath)) {
          fs.unlinkSync(documentPath);
        }
        throw new Error('Could not extract document number or date of birth from the uploaded document. Please ensure the document is clear and contains these fields.');
      }
      ...
      if (!validation.isValid) {
        ...
        // Return a structured error for the API route to use
        const errorObj = new Error('Document validation failed.');
        errorObj.validation = validation;
        throw errorObj;
      } else {
        logger.info('✅ OCR validation PASSED - All fields verified', { matches: validation.matches });
      }
    } catch (ocrError) {
      logger.error('❌ OCR/Document parsing error', { error: ocrError.message, wallet: walletAddress });
      if (fs.existsSync(documentPath)) {
        fs.unlinkSync(documentPath);
      }
      throw new Error(ocrError.message || 'Document validation failed');
    }
