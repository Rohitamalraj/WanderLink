...
export function DocumentVerificationForm({ onVerified }: { onVerified?: () => void }) {
  ...
  const handleVerifyAndEncrypt = async () => {
    ...
    try {
      ...
      if (!verifyResponse.ok || !verifyData.success) {
        // Handle validation errors returned by backend - display inline and don't navigate
        const errors = verifyData.validation?.errors || []
        const warnings = verifyData.validation?.warnings || []
        if (errors.length) {
          setValidationErrors(errors)
          toast.error('Document verification failed. See errors.', { id: 'verify', duration: 8000 })
        } else {
          const msg = verifyData.error || 'Document verification failed'
          setValidationErrors([msg])
          toast.error(msg, { id: 'verify', duration: 8000 })
        }
        setIsVerifying(false)
        return
      }
      ...
      console.log('🔄 Step 2: Starting Lighthouse encryption...')
  ...
  profilePhoto: profilePreview,
      ...
      console.log('📦 KYC data prepared, uploading to Lighthouse...')
      console.log('🔍 WalletClient status:', walletClient ? 'Available' : 'Missing')
      
      // Encrypt and upload to Lighthouse (no backend fallback)
      let cid: string | null = null
      try {
        console.log('🔐 Calling lighthouseService.encryptAndUpload...')
        toast.loading('Please approve the signature request in your wallet...', { id: 'wallet-sign' })
        
        // Add timeout wrapper (60 seconds for wallet interaction)
        const uploadPromise = lighthouseService.encryptAndUpload(
          JSON.stringify(kycData),
          walletClient
        )
        
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Lighthouse upload timeout (60s) - Did you approve the wallet signature?')), 60000)
        )
      ...
      console.log('🔄 Step 3: Starting ZK proof generation...')
      ...
      try {
        ...
        const proofRes = await fetch(${BACKEND_URL}/api/identity/generate-zk-proof, {
          ...
          body: JSON.stringify({ 
            ...
            passportNumber: documentNumber, // passport/document number
            residentialAddress, // residential address (backend calls it 'address' in second param)
            dateOfBirth,
            ageVerified
          })
        })
      ...
      try {
        // Ensure we have a Lighthouse CID before attempting to mint
        if (!cid) {
          console.error('❌ Cannot mint DataCoin: Lighthouse CID is missing')
          toast.error('DataCoin minting skipped: missing Lighthouse CID', { id: 'datacoin' })
        } else {
          ...
          {validationErrors.length > 0 && (
            <div className="p-4 rounded-md bg-red-50 border border-red-200">
              <div className="flex items-start justify-between">
                <div>
                  <h4 className="text-red-700 font-semibold">Document verification failed</h4>
                ...
                <div>
                  <Button variant="ghost" onClick={() => setValidationErrors([])} className="text-red-600">Dismiss</Button>
                </div>
              </div>
            </div>
          )}
