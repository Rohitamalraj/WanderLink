...
export class A2AMessagingService {
  ...
  async createMessageTopic(): Promise<string> {
    const transaction = new TopicCreateTransaction()
      .setTopicMemo('WanderLink Group Stake A2A Communication')
    ...
    console.log('✅ Created A2A topic:', this.topicId);
    return this.topicId;
  }
  ...
  async sendMessage(message: A2AMessage): Promise<string> {
    if (!this.topicId) {
      throw new Error('Topic not initialized. Call createMessageTopic or setTopicId first.');
    }
    ...
    console.log(`📤 Sent A2A message: ${message.type}`);
