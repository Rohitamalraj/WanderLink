...
function MintProfilePhotoNFT() {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
      <h3 className="text-xl font-black mb-4">MINT PROFILE NFT</h3>
      <Button 
        onClick={handleMint} 
        disabled={minting}
        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black border-2 border-black"
      >
        {minting ? 'MINTING...' : 'MINT PROFILE PHOTO NFT'}
      </Button>
      {txHash && (
        <div className="mt-4 p-3 bg-green-50 rounded-lg border-2 border-green-500">
          <p className="text-sm font-bold text-green-800">Minted! Tx: <code className="text-xs">{txHash.slice(0, 10)}...</code></p>
        </div>
      )}
      {error && <div className="mt-4 text-red-600 text-sm font-bold">{error}</div>}
    </div>
  )
}
...
export default function DashboardPage() {
  ...
  // State for real user name from verification
  ...
  // Fetch real user name from localStorage when address changes
  useEffect(() => {
    if (address) {
      // Get DataCoin data from localStorage
      const dataCoinData = localStorage.getItem(`dataCoin_${address}`)
      if (dataCoinData) {
        try {
          const data = JSON.parse(dataCoinData)
          if (data.fullName) {
            setUserName(data.fullName)
            console.log('✅ User name loaded:', data.fullName)
          }
        } catch (error) {
