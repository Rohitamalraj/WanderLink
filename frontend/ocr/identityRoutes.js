...
const documentService = require('../ocr/documentService');
...
router.post('/verify-document', upload.single('document'), async (req, res) => {
  try {
    ...
    // Process document and generate ZK inputs is handled in the inner try block below
    try {
      logger.info('🔐 Stage 1: Identity verification request', {
        wallet: req.body.walletAddress
      });
      ...
      const result = await processIdentityDocument(
        {
      ...
      // If validation details are present, include them in the response
      if (error.validation) {
...
router.post('/verify-age-only', async (req, res) => {
  try {
    ...
    const dobTimestamp = dobToTimestamp(dateOfBirth);
...
router.post('/encrypt-and-store', encryptAndStore.bind(identityController));

// Distribute shares to trustees
router.post('/distribute-shares', distributeShares.bind(identityController));

// Reconstruct identity from shares
router.post('/reconstruct', reconstructIdentity.bind(identityController));

// Get share status
router.get('/share-status/:loanId', getShareStatus.bind(identityController));

// Decrypt identity from IPFS
router.post('/decrypt-from-ipfs', decryptFromIPFS.bind(identityController));
...
// If this file is used as a Next.js API route, disable bodyParser for multer
// (Not needed for Express backend)
